...
func Boot(ctx context.Context, cfg *v1alpha1.NanoKubeConfig, nodeName, selfVersion string, out io.Writer) error {
	...
	if err := renewLeavesIfStale(cfg, certsLayout(kubeadm.DefaultLayout()), nodeName, out); err != nil {
...
// renewLeavesIfStale is the per-boot leaf-renewal hook called between
// kubeadm.Ensure (which depends on a complete PKI) and startKubelet
// (which reads the rotated cert files on first start). Any leaf whose
// remaining lifetime falls below certs.RenewalThreshold is re-signed
// in place via the kubeadm renewal manager.
//
// CA renewal is intentionally out of scope: CA refresh is the
// operator's responsibility (image rebuild + reset). See spec
// docs/superpowers/specs/2026-05-04-cert-lifecycle-design.md.
func renewLeavesIfStale(cfg *v1alpha1.NanoKubeConfig, layout certs.Layout, nodeName string, out io.Writer) error {
	logf := func(format string, a ...any) { fmt.Fprintf(out, "[nanokube] "+format+"\n", a...) }

	report, err := certs.CheckLeaves(cfg, layout, nodeName)
	if err != nil {
	...
	for _, leaf := range certs.AllLeaves() {
		...
		if exp.Remaining < certs.RenewalThreshold {
	...
	logf("renewing %d expiring leaves (threshold=%s)", len(stale), certs.RenewalThreshold)
	signer := certs.NewSigner(cfg, layout, nodeName)
...
// layout. OperatorDir is unused at boot (Init's seed-copy job is done),
// but populating it keeps the type honest.
