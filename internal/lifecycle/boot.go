...
func Boot(ctx context.Context, cfg *v1alpha1.NanoKubeConfig, nodeName, selfVersion string, out io.Writer) error {
	...
	if err := rotateCertsIfStale(cfg, certsLayout(kubeadm.DefaultLayout()), nodeName, out); err != nil {
...
func rotateCertsIfStale(cfg *v1alpha1.NanoKubeConfig, layout certs.Layout, nodeName string, out io.Writer) error {
	logf := func(format string, a ...any) { fmt.Fprintf(out, "[nanokube] "+format+"\n", a...) }
	...
	leafReport, err := certs.CheckLeaves(cfg, layout, nodeName)
	if err != nil {
